...
const fetchWords = async () => {
  ...
  try {
    const response = await axios.get(
      "https://random-word-api.herokuapp.com/word?number=20000&length=15",
      {},
    );
...
<template>
  <div class="grid-cols-2 grid">
    ...
    <div class="flex">
      <div class="">
        <button
          class="h-8 rounded-full bg-blue-300 text-white px-2"
          @click="addNewWord"
        >
          Add new word
        </button>
      ...
      <div class="grid grid-cols-2">
        <ul v-if="hasWords" class="draggable-list">
          <draggable
            v-model="words"
            item-key="word"
            @change="(val) => updateLocalStorage(val)"
          >
            <template #item="{ element }">
              <li :key="element">
                <WordItem
                  :value="element"
                  :remove="() => removeWord(element)"
                />
              </li>
            </template>
          </draggable>
